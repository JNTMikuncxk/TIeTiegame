...
(function(w) {
	...
	if (cps <= 1) return '失败！建议先练习！';
        if (cps <= 2) return '失败！手速被完全碾压！';
	if (cps <= 3) return '失败！差一点点！';
	if (cps <= 4) return '平局！幸运女生眷顾了你！';
        if (cps <= 4.5) return '成功！还可以！';
	if (cps <= 5.5) return '成功！单身30年？';
        if (cps <= 7)  return '成功！单身80年？';
	if (cps <= 1.14514)  return '彩蛋！好臭的tps！';
	if (cps <= 9)  return '成功！这个手速？';
	if (cps <= 12) return 'What？你确定？';
        if (cps <= 15) return 'What？';
        return '你tm是人？';
    }
